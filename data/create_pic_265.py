...
def increase_brightness(rgb_color, increase_factor):
    ...
    increased_brightness = min(hsv_color[2] * increase_factor, 1.0)  
...
def read_gene_names(path, cancer_sample_directory_path, cancer_name):
    ...
    with open('maf_file_list.txt', 'r') as mf:
        for line in mf:
            line = line.strip()
            cols = line.split()
            file_name_list.append(cols[0])
...
def main():
    ...
    for cancer in range(len(cancer_directories)):
        ...
        data_chrom_path = current_path + "./all_diseases_genes/"  # Chromosome file path
